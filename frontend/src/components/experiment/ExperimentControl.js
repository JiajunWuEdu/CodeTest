...
function ExperimentControl(props) {
  ...
  const handleRenameDialogSubmit = () => {
    setRenameDialogOpen(false);
    axios
      .post(API_URL + "experiment/rename", { "id": props.record.id, "name": newName })
      .then(response => {
        if (response.data) {
          props.getRecords();
        }
  ...
  const handleDeleteDialogSubmit = () => {
    setDeleteDialogOpen(false);
    axios
      .get(API_URL + "experiment/delete?id=" + props.record.id)
      .then(response => {
        if (response.data) {
          props.getRecords();
        }
  ...
  return (
    <>
      <Button size="small" variant="text" onClick={handleDetailDialogClickOpen}><PageviewIcon /></Button>
      ...
      {
        detailDialogOpen ?
          <ExperimentDetail onClose={handleDetailDialogClose} experiment={props.record} refreshExperiments={props.getRecords} />
          :
          <></>
      }
    </>
